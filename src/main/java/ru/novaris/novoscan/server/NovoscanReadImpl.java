...
public class NovoscanReadImpl extends RemoteServiceServlet implements
	...
	public double getDistance(DataSensor dataSensorPrev,
		...
		try {
			 ...
			 */
			List<?> sqlResult = session
					.createSQLQuery(
							(new StringBuffer().append("SELECT cal_distance(:lat1, :lon1, :lat2, :lon2, :alt1, :alt2)")).toString())
									.setDouble("lat1", dataSensorPrev.getDasnLatitude())
									.setDouble("lon1", dataSensorPrev.getDasnLongitude())
									.setDouble("lat2", dataSensorLast.getDasnLatitude())
									.setDouble("lon2", dataSensorLast.getDasnLongitude())
									.setDouble("alt1", (dataSensorPrev.getDasnHgeo() == null) ? 0.0 : dataSensorPrev.getDasnHgeo())
									.setDouble("alt2", (dataSensorLast.getDasnHgeo() == null) ? 0.0 : dataSensorLast.getDasnHgeo())
									.list();
			session.getTransaction().commit();
			return Double.valueOf(sqlResult.get(0).toString());
		} catch (Exception e) {
