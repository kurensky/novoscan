...
CREATE OR REPLACE FUNCTION add_module_combine
...
BEGIN
  ...
  IF n_md = 0 THEN
    ...
    SELECT add_sensor(MD_UID, 2, n_md, md_name, 'Комбинированный сенсор') INTO n_sn;
    IF n_sn IS NULL THEN
      ROLLBACK;
...
ALTER FUNCTION add_module_combine
 (md_uid float
 ,md_name varchar
 ,md_imei varchar
 ,md_numb varchar
...
GRANT EXECUTE ON FUNCTION add_module_combine
 (md_uid float
 ,md_name varchar
 ,md_imei varchar
 ,md_numb varchar
...
REVOKE ALL ON FUNCTION add_module_combine
 (md_uid float
 ,md_name varchar
 ,md_imei varchar
 ,md_numb varchar
...
COMMENT ON FUNCTION add_module_combine
 (md_uid float
 ,md_name varchar
 ,md_imei varchar
 ,md_numb varchar
 ...
 ,md_spob_id int8
 ,md_spmt_id int8
 ) IS 'Процедура добавления модуля и комбинированного сенсора';
